...
def log_result(member, fruit, label, confidence, filename=None, annotated_path=None,
                source="predict", user_id=None, latency_ms=None, flagged=0):
    """Insert one prediction result. Call this right after predict_ripeness() returns."""
    conn = _connect()
    conn.execute(
        """INSERT INTO results
           (member, filename, fruit, label, confidence, annotated_path, source,
            created_at, user_id, latency_ms, flagged)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (member, filename, fruit, label, confidence, annotated_path, source,
         datetime.now().isoformat(timespec="seconds"), user_id, latency_ms, int(bool(flagged))),
    )
    conn.commit()
    conn.close()
