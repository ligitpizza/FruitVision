...
MODEL_OUT_DIR = os.path.normpath(os.path.join(BASE_DIR, "..", "..", "trained_models", "ensemble_cd"))
    ...
    for label in CLASSES:
        ...
        for path in glob.glob(os.path.join(folder, "*.*")):
            try:
                ...
                vec = np.concatenate([vec_c, vec_d])
                X.append(vec)
                y.append(label)
...
if __name__ == "__main__":
    ...
    for fruit in FRUITS:
        ...
        print(f"\nBuilding feature dataset for {fruit} (C: texture + D: gabor)...")
        ...
        out_path = os.path.join(MODEL_OUT_DIR, f"{fruit}_ensemble_cd.pkl")
    ...
    if accuracies:
        ...
        print(f"\nAll graphs saved to outputs/training/cd/")
