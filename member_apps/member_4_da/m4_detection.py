...
def detect(enhanced_image):
    ...
    tracing. Fruit surfaces are typically far more saturated than a plain
    backdrop, so thresholding the S channel of HSV tends to isolate the
    fruit even when its brightness is close to the background's.

    Same interface as every other member's detect().
    """
