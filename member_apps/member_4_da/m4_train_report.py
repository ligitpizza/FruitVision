...
TRAINING_OUT_DIR = os.path.normpath(os.path.join(BASE_DIR, "..", "..", "outputs", "training", "da"))
...
def plot_confusion_matrix(y_true, y_pred, classes, fruit):
    ...
    ax.set_title(f"{fruit.capitalize()} — Confusion Matrix (D+A)")
    plt.tight_layout()
...
def plot_accuracy_summary(accuracies):
    ...
    bars = ax.bar(fruits, scores, color="#6a1b9a")
    ax.set_ylim(0, 100)
    ax.set_ylabel("Test Accuracy (%)")
    ax.set_title("Model Accuracy by Fruit (D+A)")
