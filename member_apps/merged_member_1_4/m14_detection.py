...
def detect(enhanced_image):
    """
    merged_member_1_4's detection: Otsu threshold + Suzuki-Abe contour
    tracing (member 1's original method), with a fallback to member 4's
    HSV-saturation detector when Otsu degenerates to covering ~the whole
    frame (a real, observed failure mode -- e.g. a multi-apple photo on a
    plain background, or a photo lacking a clean intensity gap for Otsu to
    split on). Without this fallback, the whole photo silently gets
    classified as "one fruit" instead of raising an error or cropping
    correctly.
