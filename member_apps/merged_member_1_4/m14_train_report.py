...
"""
Plot generators for m14_train.py.
Everything gets saved under outputs/training/merged_1_4/ so it doesn't
clash with other members' plots or outputs/reports/ (PDF reports +
dashboard charts).
"""
import os
import json
...
TRAINING_OUT_DIR = os.path.normpath(os.path.join(BASE_DIR, "..", "..", "outputs", "training", "merged_1_4"))
...
def plot_confusion_matrix(y_true, y_pred, classes, fruit):
    ...
    ax.set_title(f"{fruit.capitalize()} — Confusion Matrix (Merged 1+4)")
    plt.tight_layout()
...
def plot_accuracy_summary(accuracies):
    ...
    ax.set_title("Model Accuracy by Fruit (Merged 1+4)")
