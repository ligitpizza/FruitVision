...
Reused as-is from member_1_ab / merged_member_1_4: rectifies a cropped
fruit image by padding it to a square canvas (preserving aspect ratio)
before resizing to target_size, then reports relative-scale (frame-fraction)
normalization info since no physical reference object exists in the
current dataset.
"""
import cv2
import numpy as np
