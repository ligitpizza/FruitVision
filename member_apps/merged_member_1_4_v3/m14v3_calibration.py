"""
Calibration for m14v3: reused as-is from member 4 -- affine deskewing
(finds the fruit's minimum-area rotated bounding rectangle and warps the
crop so that rectangle's long axis is horizontal) before the same
pad-to-square + resize every member does. Combining calibration this way
is a "free win" over member 1's plain pad+resize: member 4's calibrate()
is a strict superset (deskew, then the exact same pad+resize member 1
does), so there's no tradeoff to weigh here, unlike detection/preprocessing.
"""
import cv2
import numpy as np
