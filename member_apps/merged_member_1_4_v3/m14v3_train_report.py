...
"""
Plot generators for m14v3_train.py.
Everything gets saved under outputs/training/m14v3/ so it doesn't clash
with other members' plots or outputs/reports/ (PDF reports + dashboard
charts).
"""
import os
import json
...
TRAINING_OUT_DIR = os.path.normpath(os.path.join(BASE_DIR, "..", "..", "outputs", "training", "m14v3"))
...
def plot_confusion_matrix(y_true, y_pred, classes, fruit):
    ...
    ax.set_title(f"{fruit.capitalize()} — Confusion Matrix (m14v3)")
    plt.tight_layout()
...
def plot_accuracy_summary(accuracies):
    ...
    bars = ax.bar(fruits, scores, color="#00695c")
    ax.set_ylim(0, 100)
    ax.set_ylabel("Test Accuracy (%)")
    ax.set_title("Model Accuracy by Fruit (m14v3)")
...
def save_classification_report(y_true, y_pred, classes, fruit):
    ...
    per-class performance can be read programmatically later.
    """
    out_dir = _ensure_out_dir()
