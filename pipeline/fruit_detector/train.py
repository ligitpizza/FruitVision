...
Note on the reported mAP: it's computed against datasets/yolo_fruit_detect/'s
own pseudo-labels (generated by the classical detector this model replaces),
so it measures agreement with that pseudo-labeler on close-up single-fruit
crops, not field accuracy on live webcam scenes with backgrounds/people/hands.
Treat it as a training sanity check, not a deployment accuracy claim.

Note on resuming an interrupted run: Ultralytics checkpoints last.pt every
epoch, so if training is interrupted, resume from it directly rather than
re-running this script from scratch (which would discard progress, though
the default exist_ok=True does still reuse the run directory):
    YOLO("pipeline/fruit_detector/runs/fruit_detect/weights/last.pt").train(resume=True)
"""
import os
import shutil
import argparse
