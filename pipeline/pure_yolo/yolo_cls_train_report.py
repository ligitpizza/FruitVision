...
"""
Plot generators for yolo_cls_train.py.
...
TRAINING_OUT_DIR = os.path.normpath(os.path.join(BASE_DIR, "..", "..", "outputs", "training", "yolo_pure"))
...
def plot_confusion_matrix(y_true, y_pred, classes, fruit):
    ...
    ax.set_title(f"{fruit.capitalize()} — Confusion Matrix (YOLO-cls)")
    plt.tight_layout()
...
def plot_accuracy_summary(accuracies):
    ...
    bars = ax.bar(fruits, scores, color="#37474f")
    ax.set_ylim(0, 100)
    ax.set_ylabel("Test Accuracy (%)")
    ax.set_title("Model Accuracy by Fruit (YOLO-cls)")
