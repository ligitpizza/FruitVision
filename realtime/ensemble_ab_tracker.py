# used ensemble_ab (Member 1: colour+shape) -- real-time engine
"""
...
hard-wired to Member 1's own predict_ripeness() (colour+shape SVM) instead
...
including in the mango fallback path -- selecting "Ensemble AB" for mango
must show Member 1 alone, not secretly the full 4-member vote.
"""
import os
import sys
import json
...
MEMBER_DIR = os.path.join(PROJECT_ROOT, "member_apps", "member_1_ab")
...
def _log_fps(elapsed_seconds):
    ...
    if len(_frame_times) == FPS_LOG_EVERY_N_FRAMES:
        ...
        print(f"[bench][ensemble_ab_tracker] avg FPS over last {FPS_LOG_EVERY_N_FRAMES} frames: "
...
def _record_classification(crop, fruit_type, label, confidence, tag, cleaned_img=None):
    ...
    filter_photos = filter_photos_single(cleaned_img, "ab", os.path.basename(snapshot_path), OUTPUTS_DIR)

    log_result(
        member="ensemble_ab_realtime",
        fruit=fruit_type,
        label=label,
        confidence=confidence_pct,
        filename=os.path.basename(snapshot_path),
        annotated_path=os.path.relpath(snapshot_path, OUTPUTS_DIR),
        source="realtime_ensemble_ab",
...
def _classify_crop(crop, fruit_type):
    """Runs one crop through Member 1's own SVM (colour+shape). Swaps in for
    svm_yolo_tracker.py's predict_ensemble() call -- this file is
    single-member only, on purpose."""
    try:
        label, confidence, _bbox, cleaned, _proba = m1_predict_ripeness(crop, fruit_type)
