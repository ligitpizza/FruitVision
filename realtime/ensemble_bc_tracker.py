# used ensemble_bc (Member 2: shape+texture) -- real-time engine
"""
...
hard-wired to Member 2's own predict_ripeness() (shape+texture SVM) instead
...
including in the mango fallback path -- selecting "Ensemble BC" for mango
must show Member 2 alone, not secretly the full 4-member vote.
"""
import os
import sys
import json
...
MEMBER_DIR = os.path.join(PROJECT_ROOT, "member_apps", "member_2_bc")
sys.path.append(PROJECT_ROOT)
sys.path.append(os.path.join(PROJECT_ROOT, "member_apps"))
sys.path.append(MEMBER_DIR)

from member_apps.member_2_bc.m2_predict import predict_ripeness as m2_predict_ripeness, NotAFruitError
from member_apps.member_2_bc.m2_preprocessing import clean
from member_apps.member_2_bc.m2_detection import detect as classical_detect
...
def _log_fps(elapsed_seconds):
    ...
    if len(_frame_times) == FPS_LOG_EVERY_N_FRAMES:
        ...
        print(f"[bench][ensemble_bc_tracker] avg FPS over last {FPS_LOG_EVERY_N_FRAMES} frames: "
...
def _record_classification(crop, fruit_type, label, confidence, tag, cleaned_img=None):
    ...
    filter_photos = filter_photos_single(cleaned_img, "bc", os.path.basename(snapshot_path), OUTPUTS_DIR)

    log_result(
        member="ensemble_bc_realtime",
        fruit=fruit_type,
        label=label,
        confidence=confidence_pct,
        filename=os.path.basename(snapshot_path),
        annotated_path=os.path.relpath(snapshot_path, OUTPUTS_DIR),
        source="realtime_ensemble_bc",
...
def _classify_crop(crop, fruit_type):
    """Runs one crop through Member 2's own SVM (shape+texture). Swaps in for
    svm_yolo_tracker.py's predict_ensemble() call -- this file is
    single-member only, on purpose."""
    try:
        label, confidence, _bbox, cleaned, _proba = m2_predict_ripeness(crop, fruit_type)
...
def process_frame(frame, fruit_type, frame_idx):
    ...
    status = "Tracking fruit... (Ensemble BC)" if detected_any else "No fruit detected"
