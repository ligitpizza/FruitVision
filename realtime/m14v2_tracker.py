# used merged_member_1_4_v2 (colour+shape+gabor+texture, single SVM) -- real-time engine
"""
Duplicate of merged_1_4_tracker.py's structure (same rolling-vote temporal
smoothing, same mango fallback pattern, same FPS benchmarking), but
hard-wired to merged_member_1_4_v2's own predict_ripeness() (the single SVM
trained on colour+shape+gabor+texture) instead of any 4-member vote.

Deliberately NOT calling predict_ensemble() anywhere in this file, including
in the mango fallback path -- selecting "Merged 1+4 v2" for mango must show
this model alone.
"""
import os
import sys
...
MEMBER_DIR = os.path.join(PROJECT_ROOT, "member_apps", "merged_member_1_4_v2")
...
def _log_fps(elapsed_seconds):
    ...
    if len(_frame_times) == FPS_LOG_EVERY_N_FRAMES:
        ...
        print(f"[bench][m14v2_tracker] avg FPS over last {FPS_LOG_EVERY_N_FRAMES} frames: "
...
def _classify_crop(crop, fruit_type):
    """Runs one crop through merged_member_1_4_v2's own SVM
...
def process_frame(frame, fruit_type, frame_idx):
    ...
    status = "Tracking fruit... (Merged 1+4 v2)" if detected_any else "No fruit detected"
