# used merged_member_1_4_v3 (Otsu+HSV union detect, deskew calibrate, colour+shape+gabor+texture) -- real-time engine
"""
Duplicate of m14v2_tracker.py's structure (same rolling-vote temporal
smoothing, same mango fallback pattern, same FPS benchmarking), but
hard-wired to merged_member_1_4_v3's own predict_ripeness() (the single SVM
trained on colour+shape+gabor+texture, with detection combining member 1's
Otsu box + member 4's HSV-saturation box and calibration using member 4's
deskew) instead of any 4-member vote.

Deliberately NOT calling predict_ensemble() anywhere in this file, including
in the mango fallback path -- selecting "Merged 1+4 v3" for mango must show
this model alone.
"""
import os
import sys
...
MEMBER_DIR = os.path.join(PROJECT_ROOT, "member_apps", "merged_member_1_4_v3")
sys.path.append(PROJECT_ROOT)
sys.path.append(os.path.join(PROJECT_ROOT, "member_apps"))
sys.path.append(MEMBER_DIR)

from member_apps.merged_member_1_4_v3.m14v3_predict import predict_ripeness as m14v3_predict_ripeness, NotAFruitError
from member_apps.merged_member_1_4_v3.m14v3_preprocessing import clean
from member_apps.merged_member_1_4_v3.m14v3_detection import detect as classical_detect
...
def _log_fps(elapsed_seconds):
    ...
    if len(_frame_times) == FPS_LOG_EVERY_N_FRAMES:
        ...
        print(f"[bench][m14v3_tracker] avg FPS over last {FPS_LOG_EVERY_N_FRAMES} frames: "
...
def _record_classification(crop, fruit_type, label, confidence, tag, cleaned_img=None):
    ...
    filter_photos = filter_photos_single(cleaned_img, "m14v3", os.path.basename(snapshot_path), OUTPUTS_DIR)

    log_result(
        member="m14v3_realtime",
        fruit=fruit_type,
        label=label,
        confidence=confidence_pct,
        filename=os.path.basename(snapshot_path),
        annotated_path=os.path.relpath(snapshot_path, OUTPUTS_DIR),
        source="realtime_m14v3",
...
def _classify_crop(crop, fruit_type):
    """Runs one crop through merged_member_1_4_v3's own SVM
    ...
    try:
        label, confidence, _bbox, cleaned, _proba = m14v3_predict_ripeness(crop, fruit_type)
...
def process_frame(frame, fruit_type, frame_idx):
    ...
    status = "Tracking fruit... (Merged 1+4 v3)" if detected_any else "No fruit detected"
