# used merged_member_1_4 (Member 1 colour+shape + Member 4 gabor, single SVM) -- real-time engine
"""
Duplicate of ensemble_ab_tracker.py's structure (same rolling-vote temporal
smoothing, same mango fallback pattern, same FPS benchmarking), but
hard-wired to merged_member_1_4's own predict_ripeness() (the single SVM
trained on colour+shape+gabor combined) instead of any 4-member vote.

Deliberately NOT calling predict_ensemble() anywhere in this file, including
in the mango fallback path -- selecting "Merged 1+4" for mango must show
this model alone.
"""
import os
import sys
...
MEMBER_DIR = os.path.join(PROJECT_ROOT, "member_apps", "merged_member_1_4")
sys.path.append(PROJECT_ROOT)
sys.path.append(os.path.join(PROJECT_ROOT, "member_apps"))
sys.path.append(MEMBER_DIR)

from member_apps.merged_member_1_4.m14_predict import predict_ripeness as m14_predict_ripeness, NotAFruitError
from member_apps.merged_member_1_4.m14_preprocessing import clean
from member_apps.merged_member_1_4.m14_detection import detect as classical_detect
...
def _log_fps(elapsed_seconds):
    ...
    if len(_frame_times) == FPS_LOG_EVERY_N_FRAMES:
        ...
        print(f"[bench][merged_1_4_tracker] avg FPS over last {FPS_LOG_EVERY_N_FRAMES} frames: "
...
def _record_classification(crop, fruit_type, label, confidence, tag, cleaned_img=None):
    ...
    filter_photos = filter_photos_single(cleaned_img, "merged_1_4", os.path.basename(snapshot_path), OUTPUTS_DIR)

    log_result(
        member="merged_1_4_realtime",
        fruit=fruit_type,
        label=label,
        confidence=confidence_pct,
        filename=os.path.basename(snapshot_path),
        annotated_path=os.path.relpath(snapshot_path, OUTPUTS_DIR),
        source="realtime_merged_1_4",
...
def _classify_crop(crop, fruit_type):
    """Runs one crop through merged_member_1_4's own SVM (colour+shape+gabor).
    Swaps in for svm_yolo_tracker.py's predict_ensemble() call -- this file
    is single-model only, on purpose."""
    try:
        label, confidence, _bbox, cleaned, _proba = m14_predict_ripeness(crop, fruit_type)
...
def process_frame(frame, fruit_type, frame_idx):
    ...
    status = "Tracking fruit... (Merged 1+4)" if detected_any else "No fruit detected"
